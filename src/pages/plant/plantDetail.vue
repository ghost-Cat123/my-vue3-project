<template>
  <view class="plantInfo">
    <image class="plantImg" :src=plantInfo.plantImg></image>
    <button class="updateBtn" type="primary" size="mini" @click="updatePlant()">修改</button>
    <view class="cName">{{ plantInfo.plantCname }}</view>
    <view class="eName">{{ plantInfo.plantEname }}</view>
    <view class="intro">{{ plantInfo.plantIntro }}</view>
    <view class="age">年龄：{{ plantInfo.plantAge }}</view>
    <view class="loc">位置：{{ plantInfo.plantLoc }}</view>
  </view>
  ...
  <view class="task">
    ...
    <view class="taskCard" v-for="task in taskList" :key="task.taskId">
      <view class="taskBox">
        <image name="taskImg" class="taskImg" src="@/static/icon/plantTask.png"></image>
  ...
  <view class="tools">
    ...
    <view class="noteBox">
      ...
      <textarea class="note" v-model="note.plantNote" maxlength="400" placeholder="请输入您的笔记" @blur="bindTextAreaBlur"
...
export default {
  ...
  methods: {
    ...
    async addTask() {
      this.taskFormData.plantId = this.id.plantId
      const res = await postAddPlantTaskAPI(this.taskFormData)
      if (res.code === 1) {
    ...
    async updateTask() {
      const res = await postUpdatePlantTaskAPI(this.taskFormData)
      if (res.code === 1) {
        // 修改前端
        this.taskList.forEach((task) => {
    ...
    async saveNote() {
      this.note.plantId = this.id.plantId
      const res = await postPlantNoteUpdateAPI(this.note)
      if (res.code === 1) {
        uni.showToast({
          title: '笔记更新成功',
          icon: 'success',
